...
export async function GET() {
  ...
  const [preferences, searches, favorites, profile, matches] = await Promise.all([
    ...
    supabase
      ...
      .select("id, query_text, selected_categories, filters_applied, result_count, results_clicked, first_click_position, source, created_at")
      .eq("user_id", userId)
  ...
  return new NextResponse(JSON.stringify(payload, null, 2), {
    headers: {
      ...
      "Content-Disposition": 'attachment; filename="auktio-min-data-export.json"',
    },
  });
}